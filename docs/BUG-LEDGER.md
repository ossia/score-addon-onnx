...
| A5 | S2 — **fixed** | design | There is no mel or STFT frontend, so vocoders and spectrogram models get raw samples in a `[1,80,T]` tensor. | Done: a `[1,F,T]` input with F > 8 bins and a waveform output is fed the log-mel spectrogram of the incoming audio (`Onnx/helpers/MelFrontend.hpp`: HiFi-GAN's settings, n_fft 1024, hop 256, 0–8 kHz, Slaney filterbank, natural log floored at 1e-5; 32 frames per block, the n_fft − hop samples before each block replacing the reflect padding; the model rate defaults to 22050 Hz). The Overlap input does not apply there. Spectrogram inputs that are not mel vocoders (STFT bins 2^k + 1, as in `unet_source_separation`, or a spectrogram output) are refused once with a message instead of ORT throwing on every block. Vocoders with other mel settings (Vocos: 24 kHz, 100 bins) would need parameters. Test: `test_audio_mel.cpp` (the frontend matches an independent numpy reference within 1e-3, and a log10 mutation is off by 6.5; HiFi-GAN turns a 440 Hz sine into audio whose 440 Hz power is over 10× that at 220–2000 Hz, where the old code was silent; the U-Net separator is refused). |
...
| F3 | S3 | design | SmolVLM (idefics3: 5-D `pixel_values`, tiling, `pixel_attention_mask`, 64 tokens per tile) and gemma3 (896² input, 256 soft tokens between `<start_of_image>`/`<end_of_image>`) need per-architecture preprocessing and prompt expansion; FastVLM feeds the native image size with no resize. | Per-architecture image preprocessing from `preprocessor_config.json` and placeholder expansion. See the F1 diagnosis. |
