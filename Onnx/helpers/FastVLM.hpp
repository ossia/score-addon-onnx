...
private:
    ...
    std::vector<float> createMultimodalEmbeddings(std::span<int64_t> tokenIds,
                                                  std::span<float> imageFeatures);
    std::vector<int64_t> generateWithONNXDecoder(std::span<float> embeddings,
                                                 int maxTokens,
    ...
    std::vector<std::vector<Ort::Float16_t>> keyCache = std::vector<std::vector<Ort::Float16_t>>(24);
    std::vector<std::vector<Ort::Float16_t>> valueCache = std::vector<std::vector<Ort::Float16_t>>(
        24);
    std::vector<std::vector<int64_t>> cacheShapes = std::vector<std::vector<int64_t>>(24);
