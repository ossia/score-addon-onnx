...
struct TokenizerConstants
{
    ...
    static constexpr int IMAGE_TOKEN_INDEX = 151646; // <image> token from tokenizer config
    static constexpr int IGNORE_INDEX = -100;        // For labels to ignore during training
...
std::string FastVLMInference::generateResponse(const QImage &image, const std::string &prompt, float temperature)
{
    try {
        ...
        auto multimodalEmbeddings = createMultimodalEmbeddings(tokenIds, imageFeatures);

        // Generate tokens using our working ONNX decoder with temperature sampling
        auto generatedTokens = generateWithONNXDecoder(multimodalEmbeddings, 500, temperature);
        ...
        if (assistantPos != std::string::npos) {
            response = response.substr(assistantPos + 22); // Skip "<|im_start|>assistant\n"
        ...
        throw std::runtime_error(std::format("Manual multimodal inference failed: {}", e.what()));
    }
}

std::vector<float> FastVLMInference::runVisionEncoder(std::span<float> imageData)
{
    try {
        auto memoryInfo = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
        
        std::vector<int64_t> inputShape = {1, 3, ImagePreprocessor::TARGET_SIZE, ImagePreprocessor::TARGET_SIZE};
        ...
        auto outputName = visionEncoderSession->GetOutputNameAllocated(0, allocator);
        
        const char* inputNames[] = {inputName.get()};
        const char* outputNames[] = {outputName.get()};

        auto outputs = visionEncoderSession->Run(Ort::RunOptions{nullptr},
                                                 inputNames,
                                                 &inputTensor,
                                                 1,
                                                 outputNames,
        ...
        size_t outputSize = std::accumulate(outputShape.begin(), outputShape.end(), 
                                           1ULL, std::multiplies<size_t>());
        
        return std::vector<float>(outputTensor, outputTensor + outputSize);
    } catch (const Ort::Exception& e) {
        throw std::runtime_error(std::format("Vision encoder failed: {}", e.what()));
    }
}

std::vector<float> FastVLMInference::runEmbedTokens(std::span<int64_t> tokenIds)
{
    try {
        auto memoryInfo = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
        
        std::vector<int64_t> inputShape = {1, static_cast<int64_t>(tokenIds.size())};
        auto inputTensor = Ort::Value::CreateTensor<int64_t>(memoryInfo,
        ...
        auto outputs = embedTokensSession->Run(Ort::RunOptions{nullptr},
                                               inputNames,
                                               &inputTensor,
                                               1,
                                               outputNames,
...
std::string FastVLMInference::decodeTokens(std::span<int64_t> tokens) const
{
    try {
        ...
        extError_t result = OrtxDetokenize1D(tokenizer, ortxTokens.data(), ortxTokens.size(), &stringArray);
        if (result != kOrtxOK) {
            throw std::runtime_error(std::format("Token decoding failed: {}", OrtxGetLastErrorMessage()));
        }
        ...
        if (result != kOrtxOK) {
            ...
            throw std::runtime_error(std::format("Failed to get decoded string: {}", OrtxGetLastErrorMessage()));
        }
        ...
        throw std::runtime_error(std::format("Token decoding failed: {}", e.what()));
    }
}

std::vector<int64_t> FastVLMInference::tokenizeImagePrompt(const std::string &prompt) const
{
    try {
        ...
        while (pos < prompt.length()) {
            size_t imagePos = prompt.find(TokenizerConstants::DEFAULT_IMAGE_TOKEN, pos);
        ...
        for (size_t i = 0; i < chunks.size(); ++i) {
            if (i > 0 && i % 2 == 1) {
                ...
                extError_t result = OrtxTokenize(tokenizer, inputTexts, 1, &tokenArray);
                if (result != kOrtxOK) {
                    throw std::runtime_error(std::format("Tokenization failed: {}", OrtxGetLastErrorMessage()));
                }
                
                // Get tokens from the first sequence
                const extTokenId_t* tokenData = nullptr;
                size_t tokenCount = 0;
                result = OrtxTokenId2DArrayGetItem(tokenArray, 0, &tokenData, &tokenCount);
                if (result != kOrtxOK) {
                    OrtxDispose((OrtxObject**)&tokenArray);
                    throw std::runtime_error(std::format("Failed to get tokens: {}", OrtxGetLastErrorMessage()));
                }
                
                // Handle BOS token - only add at the very beginning
                size_t startIdx = 0;
                if (i == 0 && tokenCount > 0 && tokenData[0] == TokenizerConstants::BOS_TOKEN_ID) {
        ...
        throw std::runtime_error(std::format("Image tokenization failed: {}", e.what()));
    }
}

std::string FastVLMInference::createPromptTemplate(std::string_view userPrompt) const
{
...
std::vector<float> FastVLMInference::createMultimodalEmbeddings(std::span<int64_t> tokenIds,
                                                                std::span<float> imageFeatures)
{
    try {
        ...
        multimodalEmbeddings.reserve((tokenIds.size() - 1 + imagePatchCount) * hiddenSize);
        
        for (size_t i = 0; i < tokenIds.size(); ++i) {
            if (tokenIds[i] == TokenizerConstants::IMAGE_TOKEN_INDEX) {
                // Replace single image token with 256 patch embeddings
                multimodalEmbeddings.insert(multimodalEmbeddings.end(), 
                                          imageFeatures.begin(), imageFeatures.end());
            } else {
                ...
                multimodalEmbeddings.insert(multimodalEmbeddings.end(),
                                          tokenEmbedding.begin(), tokenEmbedding.end());
            }
        }
        
        #ifndef NDEBUG
        std::cout << std::format("Final multimodal embeddings size: {} (expected sequence_length * {})\n",
                                multimodalEmbeddings.size(), hiddenSize);
        #endif
        
        return multimodalEmbeddings;
        
    } catch (const std::exception& e) {
        throw std::runtime_error(std::format("Failed to create multimodal embeddings: {}", e.what()));
    }
}

std::vector<int64_t> FastVLMInference::generateWithONNXDecoder(std::span<float> embeddings,
                                                               int maxTokens, float temperature)
{
    try {
        ...
        auto createEmbedsTensor =
            [&memoryInfo](std::span<float> embeds, size_t seqLen, size_t hiddenSize) {
        ...
        auto createAttentionTensor = [&memoryInfo, this](size_t seqLen) {
            ...
            return Ort::Value::CreateTensor<int64_t>(memoryInfo,
        ...
        auto createPositionTensor = [&memoryInfo, this]() {
            std::vector<int64_t> shape = {1, static_cast<int64_t>(reusablePositionIds.size())};
            return Ort::Value::CreateTensor<int64_t>(memoryInfo,
        ...
        auto createKVCacheTensor = [&memoryInfo](const std::vector<Ort::Float16_t> &cache,
                                                 const std::vector<int64_t> &shape) {
            return Ort::Value::CreateTensor<Ort::Float16_t>(memoryInfo,
        ...
        // Run the decoder
        auto outputs = decoderSession->Run(Ort::RunOptions{nullptr},
        ...
        auto sampleToken =
            [temperature](const Ort::Value &logitsTensor) -> std::pair<int64_t, float> {
            ...
            float maxLogit = *std::max_element(logitsData + lastPos, logitsData + lastPos + vocabSize);
            ...
            return {static_cast<int64_t>(sampledToken), logitsData[lastPos + sampledToken]};
        };
        ...
        for (int layer = 0; layer < 24; ++layer) {
            ...
            cacheShapes[layer] = {keyShape[0], keyShape[1], keyShape[2], keyShape[3]};
        ...
        for (int step = 1; step < maxTokens; ++step) {
            ...
            for (int layer = 0; layer < 24; ++layer) {
                nextInputs.push_back(createKVCacheTensor(keyCache[layer], cacheShapes[layer]));
                nextInputs.push_back(createKVCacheTensor(valueCache[layer], cacheShapes[layer]));
            }

            // Run decoder for next token
            auto nextOutputs = decoderSession->Run(Ort::RunOptions{nullptr},
                                                  decoderInputNamePtrs.data(),
                                                  nextInputs.data(),
